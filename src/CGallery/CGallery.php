    ...
    public function __construct() {
        ...
        function readAllItemsInDir($path, $validImages = array('png', 'jpg', 'jpeg', 'gif')) {
          ...
          foreach($files as $file) {
            ...
            elseif(is_dir($file)) {
              $item    = "<img src='http://dbwebb.se/kod-exempel/skapa-ett-galleri-av-dina-bilder-med-php/img/folder.png' alt=''/>";
              $caption = basename($file) . '/';
            }
            else {
              continue;
...
EOD;
        ...
        if(is_dir($pathToGallery)) {
          $this->gallery = readAllItemsInDir($pathToGallery);
        }
        else if(is_file($pathToGallery)) {
          $this->gallery = readItem($pathToGallery);
        }



        //
        // Prepare content and store it all in variables in the Branax container.
        //
        $this->breadcrumb = createBreadcrumb($pathToGallery);
